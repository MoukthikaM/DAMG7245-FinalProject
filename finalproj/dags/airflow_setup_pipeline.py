...
from airflow_tasks import bulk_extract,bulk_load,bulk_train_Linear_regression_task,bulk_train_randomforest_regression_task,reset_database,bulk_predict_bestmodel,bulk_train_K_nearestneighbours_task
import pendulum
import json
default_args = {
    'owner': 'mouk',
...
@dag(
    ...
    catchup=False,
    tags=['HousePricePredictionSetup'])
def houseprice_setup_taskflow():
    import uuid
    import json
    ...
    state_dict=bulk_extract(state_dict)
    state_dict=bulk_load(state_dict)
    print(state_dict)
